...
#define EMUVERSION "V0.1.6 2026-01-05"
...
const MItem fnList9[GAME_COUNT] = {
	{"Yie Ar Kung Fu (US)", quickSelectGame},
	{"Yie Ar Kung Fu (JP)", quickSelectGame},
};
...
const char *getBonusText() {
	return bonusTxt[(g_dipSwitch1>>3)&1];
}
/// Demo sound on/off
void demoSet() {
	g_dipSwitch1 ^= 0x80;
...
const char *getDifficultText() {
	return diffTxt[(g_dipSwitch1>>4)&3];
}
